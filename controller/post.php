...
if (isset($_POST) && isset($_POST['image'])
    ...
    && file_exists($_SERVER["DOCUMENT_ROOT"] . $FILTERS_LOCATION . $filter))
{
    $filter = imagecreatefrompng($_SERVER["DOCUMENT_ROOT"] . $FILTERS_LOCATION . $filter);
    ...
    $fileName = $sessionManager->get_logged_user_name()
        . '_' . date( 'd_m_Y.H:i:s') .'.png';
    $fileLocation = $IMAGE_STORAGE_FULL_LOCATION . $fileName;
    $imageLink = $IMAGE_STORAGE_LOCATION . $fileName;
